...
11. **`/loop:refine` MUST always generate three sets of prompts and MUST let the user choose; it MUST NOT choose for the user** (v4.3): receive a one-sentence requirement → 8-dimension deep analysis of missing points → use AskUserQuestion to dynamically ask 3-6 follow-up questions once → **always** produce three sets (standard/compact/advanced strict) → use AskUserQuestion to let the user choose one → write to `.loop/refined-prompt.md`. The agent MUST NOT decide the version on its own, MUST NOT guess missing dimensions (if missing, ask), and MUST NOT skip the follow-up step.
    <!-- 中文译注：铁律 11——/loop:refine 总是生成三套提示词，必须让用户选，不得替选（v4.3）：一句话需求→8 维度分析→AskUserQuestion 一次追问 3-6 个→总是产出三套→让用户选→写 refined-prompt.md；不擅自定版、不臆测缺失维度、不跳过追问。 -->
...
4. Use AskUserQuestion to let the user choose one set — **the agent does not choose for the user**.
...
- [ ] **/loop:refine**: did you always generate three sets (standard/compact/advanced strict) and use AskUserQuestion to let the user choose (not choose for them)?
