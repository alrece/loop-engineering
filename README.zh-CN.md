...
**版本 v4.4** · 2026-07 · [更新日志](#十二版本与更新日志) · [安装指南](INSTALL.md)
...
**核心定位**：全自动长程工程闭环 + 对抗性质量门 + 前端生命周期全覆盖 + 提示词专业化优化 + **多 AI Agent 端部署**。
...
| 提示词专业化优化 | `/loop:refine` 一句话需求 → 8 维度分析 → 动态追问 → 三套供选（v4.3）|
| **多端部署** | Cursor / Codex / Gemini / Codebuddy 适配层 + 一键安装脚本（v4.4 新增）|

### 版本演进总览

| 版本 | 发布 | 核心改进 | 触发动机 |
|------|------|---------|---------|
| **v4.4** | 2026-07 | 多 AI Agent 端部署（Cursor/Codex/Gemini/Codebuddy 适配层 + install.sh） | 跨 agent CLI 使用需求 |
...
#### v4.4（当前，2026-07）— 多 AI Agent 端部署

**新增能力**：
- ✨ 多 agent 适配层：支持 **Cursor**、**Codex**、**Gemini CLI**、**Codebuddy**（Claude Code/ZCode 之外的 4 个 agent CLI）
- ✨ `adapters/install.sh` 一键安装脚本——自动检测已安装的 agent 端，部署对应 adapter
- ✨ Gemini + Codebuddy 共享 `~/.agents/skills/` 的同一份部署（通过 symlink 一份服务两端）

**适配设计**（详见 [`adapters/README.md`](adapters/README.md)）：
- **Cursor**：`<cursor_skill_adapter>` A/B/C/D 四节（工具映射：Edit→StrReplace、Bash→Shell；AskUserQuestion→对话式编号列表；Task→Task(subagent_type=...)；SlashCommand→inline 执行）
- **Codex**：`<codex_skill_adapter>` A/B/C 三节（AskUserQuestion→request_user_input 含 #3018 fallback；Task→spawn_agent；`/cmd`→`$cmd`）
- **Gemini+Codebuddy**（共享）：inline 工作流（无 `@include`）、纯文本问答、多模型对抗降级为单模型

**能力降级矩阵**：非原生端保留核心能力（编排、对抗门脚本、状态机），但交互式功能降级。详见 [INSTALL.md 多端部署章节](INSTALL.md)。

**新增文件**：
- `adapters/README.md` — adapter 清单 + 能力矩阵
- `adapters/install.sh` — 一键安装脚本
- `adapters/cursor/SKILL.md` — Cursor 适配版
- `adapters/codex/SKILL.md` — Codex 适配版
- `adapters/gemini-codebuddy/SKILL.md` — Gemini+Codebuddy 共享适配版（inline 工作流）

**修改文件**：
- INSTALL.md：新增「Deploy to Multiple AI Agents (v4.4)」章节
- README.md / README.zh-CN.md：版本升至 v4.4 + 本版本日志条目

#### v4.3（2026-07）— 提示词专业化优化器
