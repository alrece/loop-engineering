...
## A. Skill Invocation
- This skill is invoked when the user mentions `loop-engineering` or describes a task matching this skill (engineering closed loop, adversarial quality gate, iterative project delivery).
- Treat all user text after the skill mention as `{{GSD_ARGS}}`.
- If no arguments are present, treat `{{GSD_ARGS}}` as empty.

## B. User Prompting
When the workflow needs user input (follow-up questions, 3-prompt selection, blocker confirmation), prompt the user conversationally:
- Present options as a numbered list in your response text
- Ask the user to reply with their choice
- For multi-select, ask for comma-separated numbers
- For the /loop:refine 3-prompt selection: fully display all three prompts as code blocks first, then present a numbered list (1=Standard, 2=Compact, 3=Advanced strict) and wait for the user's reply

## C. Tool Usage
Use these Cursor tools when executing loop-engineering workflows:
- `Shell` for running commands (terminal operations, adversarial gate scripts)
- `StrReplace` for editing existing files (replaces Claude's `Edit`)
- `Read`, `Write`, `Glob`, `Grep`, `Task`, `WebSearch`, `WebFetch`, `TodoWrite` as needed
- NOTE: Cursor has no `SlashCommand` tool. Loop self-invocation (`--auto` chain) is done by inline execution (re-entering the workflow logic in the current session) rather than self-calling the skill.

## D. Subagent Spawning
When the workflow needs to spawn a subagent (e.g., gsd-executor for plan-by-plan execution):
...
Design follows the GSD style: SKILL.md is kept minimal, logic is externalized into workflow files, and looping is implemented via @ includes + inline execution (cursor has no SlashCommand, so the loop is re-entered inline within the current session).
<!-- 中文译注：设计遵循 GSD 风格——SKILL.md 极小，逻辑外置到 workflow 文件，用 @ include + inline 执行实现循环（cursor 端无 SlashCommand，改为 inline 重新进入 workflow）。 -->
</objective>

<execution_context>
@$HOME/.cursor/skills/loop-engineering/AGENTS.md
@$HOME/.cursor/get-shit-done/workflows/loop-state.md
@$HOME/.cursor/get-shit-done/workflows/loop-orchestrate.md
@$HOME/.cursor/get-shit-done/workflows/loop-iterate.md
@$HOME/.cursor/get-shit-done/workflows/loop-adversarial.md
@$HOME/.cursor/get-shit-done/workflows/replicate-workflow.md
@$HOME/.cursor/get-shit-done/workflows/loop-refine.md
</execution_context>
...
2. Use conversational numbered-list prompting to **dynamically generate 3-6 follow-up questions** (targeting uncovered dimensions, asked once in parallel).
...
4. **First fully display all three prompts in the conversation** (each set's full text rendered as a code block, the user must be able to read every line), **then** present a numbered list (1=Standard, 2=Compact, 3=Advanced strict) and wait for the user to choose one set (**the agent MUST NOT choose for the user; MUST NOT skip the full display and jump to the selection**).
...
2. **--auto / --force (default v3.1)**: enter the **run_full_loop single-session inline continuous loop** of loop-orchestrate.md — the loop itself, in the main session, Reads each stage's main-tool SKILL.md + workflow and executes them inline (using Read/Write/StrReplace/Shell/conversational prompting), NOT via SlashCommand (Cursor has no such tool), NOT via Skill() (ZCode has no such tool), NOT via Agent subagents (Explore is read-only). After each stage it runs the adversarial gate + advance_loop to advance; at decision points decide_best auto-selects; only a Hard Blocker breaks. **A single invocation runs the whole way, without interruption.** `--force` additionally skips all safety gates.
3. **--interactive**: each stage is invoked via inline execution (re-entering the workflow logic in the current session); on hitting a decision/safety gate it stops and asks a human (conservative behavior).
...
5. show_and_execute: --auto/--force uses inline execution; --interactive uses inline execution with stops.
<!-- 中文译注：--next 推进——默认 --auto 全自动：进入 run_full_loop 单会话 inline 连续循环（loop 自己 Read skill 指令+主会话执行，非 SlashCommand/Skill/Agent），每环节后跑对抗门+advance_loop 前进，遇决策点 decide_best 自动选，只有硬障碍才 break，一次调用跑完全程；--interactive 用 inline 执行，撞决策/安全门即停问人。 -->
...
<runtime_note>
**Cursor limitations:**
- No `AskUserQuestion`: interactive Q&A uses conversational numbered lists (see adapter B).
- No `SlashCommand`: loop self-invocation uses inline execution.
- Multi-model adversarial (codex+gemini): degraded — Cursor runs single-model; if codex/gemini backends are configured via cc-switch, the adversarial gate script can still invoke them via Shell.
- For full capabilities (multi-model adversarial, SlashCommand chaining), use Claude Code / ZCode.
</runtime_note>
