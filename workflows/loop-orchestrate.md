...
**Route 5.5：桌面端全栈部署编排（Phase 3, step=deploy）— v4.2 新增**
条件：PLAN.md 存在，但部署配置不完整（前端服务被 profiles 隐藏或无 make deploy）。
→ 下一步：自动修复部署配置（移除 profiles 隐藏、生成 make deploy），再 `/gsd-plan-phase <N>`。
说明：确保全栈部署可用。**--auto 模式下自动修复，不停；--interactive 模式下修复失败则停问人。** 完成后 step → plan。
...
**Route 11：发布流水线（Phase 5, step=ship）**
条件：验收通过，未发布。
...
**Route 12：迭代/里程碑闭环（Phase 6, step=complete/retro）**
条件：发布完成，未做复盘。
