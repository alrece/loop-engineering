...
5. **提示词专业化优化**（v4.3 新增）：`/loop:refine` 接收一句话需求 → 8 维度深度分析 → 动态追问 3-6 个问题 → 总是生成三套（标准/精简/高阶强约束）供用户选。横切工具，不依赖 init、不推进闭环
...
**`--refine <text>`** → 执行提示词专业化优化（v4.3 新增）：完全交给 loop-refine.md workflow。
1. 接收用户的一句话需求（`<text>`），按 8 个维度深度分析缺失/模糊点
2. 用 AskUserQuestion **动态生成 3-6 个追问**（针对未覆盖维度，一次性并行追问）
3. 综合答复后**总是生成三套**提示词：标准版（日常使用）/ 精简版（迭代对话）/ 高阶强约束版（AI Agent 强化防护）
4. 用 AskUserQuestion 让用户选一套（**agent 不替选**）
5. 写入 `.loop/refined-prompt.md`（含原始/追问记录/三套全文/选定版本）
6. 提示下一步：`/office-hours`（带优化后提示词）或 `/loop:run --next`。**不自动调用，不推进闭环**（refine 是横切工具）
...
<success_criteria>
- [ ] $ARGUMENTS 正确路由到各模式（init/refine/status/retro/next/adversarial/phase/from）
